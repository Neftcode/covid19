<?php
	...
	class festivos {
		 ...
		 * @param  {String-Integer} $anio [Recibe como par�metro el a�o a validar]
		...
		public function festivos($anio='') {
			...
			$this->festivos[$anio][8][7]   = true;		// Batalla de Boyac� 7 de Agosto
			...
			$this->calculateEmiliani(8, 15);				// Asunci�n Agosto 15
			$this->calculateEmiliani(10, 12);			// Descubrimiento de Am�rica Oct 12
			...
			$this->otherDatesCalculated(43,true);		//Ascenci�n el Se�or pascua
			$this->otherDatesCalculated(60,true);		//Corpus Cristi
			$this->otherDatesCalculated(68,true);		//Sagrado Coraz�n
			
			// otras fechas importantes que no son festivos

			// $this->otherDatesCalculated(-46);		// Mi�rcoles de Ceniza
			// $this->otherDatesCalculated(-46);		// Mi�rcoles de Ceniza
		...
		/**
	     * Funci�n que mueve una fecha diferente a lunes al siguiente lunes en el calendario y se aplica a fechas que estan bajo la ley emiliani global
	     * @param  {Integer} $mes_festivo [Mes en el que se va calcular]
	     * @param  {Integer} $dia_festivo [D�a en el que se va calcular]
	     * @return {null}             	  [Sin retorno]
	     */
		...
		/**
		 * Funci�n que calcula otras fechas a partir de la pascua o fechas no festivas pero son importantes
		 * @param  {Number}  $cantidadDias   [Cantidad de d�as a sumar o restar]
		...
		/**
		 * Funci�n para verificar si la fecha obtenida por par�metro es festiva (el a�o se pasa como par�metro en el constructor)
		 * @param  {Integer} $dia [D�a a validar]
